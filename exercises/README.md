...
２つの変数間にある関係性の強弱を図る指標で、ある２つの値、２つの指標を見比べたときにある一方が上がればもう一方が下がるという関係性があったときに相関があり、これらを数値で表現したのが相関関数である

- 両方が上がる時：`正の相関`で値は`1`
- 一方が上がり一方が下がる時：`負の相関`で値は`-1`

`df`の下記カラム同士の相関係数を算出する
- 平均気温
- 降水量の合計
- 日照時間
...
def main():
    ...
    df = pd.read_csv(csv_file, sep=",")
    # print(df.columns)
    df = df[['平均気温(℃)', '降水量の合計(mm)','日照時間(時間)']][1:]
    print(df.corr())
...
- 同カラムの比較
    ...
    例えば、平均気温と平均気温の相関係数は`1.000000`であるが、同じ値を比較しているため

- 平均気温との比較

     降水量の合計の相関係数は`0.027858`となり、平均気温の上下に余り関係性はない

     日照時間の相関係数は`0.071758`となり、平均気温の上下に余り関係性はない

<a id="20"></a>

### 20. データの出力

欠損値を0で補完した`df`を`export.csv`というファイル名で出力する
